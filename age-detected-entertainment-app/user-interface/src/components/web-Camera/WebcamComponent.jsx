...
const WebcamComponent = () => {
  ...
  const noOfChancesForAgeDetector = useSelector(
    (state) => state.ageDetectorEnabeled.ageDetectorEnabelingCounter
  );
  console.log(noOfChancesForAgeDetector);
  ...
  const sendImageToServer = async (imageDataUrl) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        if (detectedAge >= 18) {
          dispatch(
            ageDetectorEnabelingSliceAction.resetAgeDetectorEnabelingCounter()
          );
          navigate("/adult");
          ...
          dispatch(
            ageDetectorEnabelingSliceAction.incrementAgeDetectorEnabelingCounter()
          );
        }
      } else {
  ...
  useEffect(() => {
    if (noOfChancesForAgeDetector >= 3) {
      toast.warn("You have exceeded the maximum Limit. Try After 2 minutes.");
      cancelAgeDetector();
      setTimeout(() => {
        dispatch(
          ageDetectorEnabelingSliceAction.resetAgeDetectorEnabelingCounter()
