...
import { userVerificationSliceAction } from "../../redux/slices/userVerificationStatus/index";
...
const Adults = () => {
  ...
  const userAnsweredCorrectly = useSelector(
    (state) => state.userVerificationStatus.userAnsweredCorrectly
  );
  ...
  const verificationHanedler = () => {
    ...
    if (point === 5) {
      dispatch(
        userVerificationSliceAction.enabelingUserVerificationForAdultMovies()
      );
    } else {
      dispatch(chatSectionEnabelingSliceAction.disabelingChatSection());
